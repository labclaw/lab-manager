...
# ---- _run_extraction background task (lines 149-206) ----


class TestRunExtraction:
    def test_doc_not_found(self, db_session, tmp_path):
        """Line 159-161: document not found during extraction."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
    ...
    def test_ocr_failure(self, db_session, tmp_path):
        """Lines 169-174: OCR fails, status set to needs_review."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
    ...
    def test_empty_ocr_text(self, db_session, tmp_path):
        """Lines 176-181: OCR returns empty text, status set to ocr_failed."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
    ...
    def test_extraction_success(self, db_session, tmp_path):
        """Lines 183-201: full extraction pipeline success."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        os.environ["EXTRACTION_MODEL"] = "test-model"
        from lab_manager.config import get_settings
        ...
        mock_extracted.document_type = "invoice"
        mock_extracted.vendor_name = "TestVendor"
    ...
    def test_extraction_failure_after_ocr(self, db_session, tmp_path):
        """Lines 195-198: extraction step fails, status set to needs_review."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
    ...
    def test_unexpected_exception(self, db_session, tmp_path):
        """Lines 202-204: unexpected error in extraction."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        fake_session = MagicMock()
        # Make scalars itself raise to trigger the outer except
        fake_session.scalars.side_effect = RuntimeError("DB connection lost")
        fake_factory = MagicMock(return_value=fake_session)
...
# ---- _index_approved_doc background task (lines 211-288) ----


class TestIndexApprovedDoc:
    def test_doc_not_found(self, tmp_path):
        """Lines 228-229: document not found for indexing."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
    ...
    def test_index_doc_record_failure(self, db_session, tmp_path):
        """Lines 231-233: index_document_record raises, handled gracefully."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        def scalars_side_effect(stmt):
            result = MagicMock()
            # First call returns doc, second returns no order
        ...
        fake_session.scalars.side_effect = scalars_side_effect
        fake_factory = MagicMock(return_value=fake_session)
        ...
        with patch(
            ...
            with patch(
                ...
                side_effect=RuntimeError("Meilisearch down"),
            ):
    ...
    def test_index_with_order_vendor_items(self, db_session, tmp_path):
        """Lines 246-283: full indexing path with order, vendor, items, products, inventory."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        # Create a mock order with vendor, items, product, inventory
        mock_vendor = MagicMock()
        mock_vendor.id = 1
        mock_oi = MagicMock()
        mock_oi.id = 10
        mock_oi.product = MagicMock()
        mock_oi.product.id = 20
        mock_order = MagicMock()
        mock_order.id = 5
        mock_order.vendor = mock_vendor
        mock_order.items = [mock_oi]

        mock_inv = MagicMock()
        mock_inv.id = 30
        mock_inv.order_item_id = 10
        ...
        def scalars_side_effect(stmt):
            ...
            if call_count["n"] == 1:
                ...
                result.all.return_value = [mock_inv]
                result.first.return_value = None
            return result
        ...
        with patch(
            ...
            with patch("lab_manager.services.search.index_document_record"):
                with patch("lab_manager.services.search.index_vendor_record"):
                    with patch("lab_manager.services.search.index_order_record"):
                        with patch(
    ...
    def test_index_vendor_record_failure(self, db_session, tmp_path):
        """Lines 250-251: index_vendor_record raises."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        with patch(
            ...
            with patch("lab_manager.services.search.index_document_record"):
                with patch(
                    ...
                    side_effect=RuntimeError("vendor idx fail"),
                ):
                    with patch("lab_manager.services.search.index_order_record"):
                        doc_mod._index_approved_doc(doc.id)
    ...
    def test_index_order_item_product_failure(self, db_session, tmp_path):
        """Lines 270-277: index_order_item_record and index_product_record raise."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        with patch(
            ...
            with patch("lab_manager.services.search.index_document_record"):
                with patch("lab_manager.services.search.index_order_record"):
                    with patch(
                        "lab_manager.services.search.index_order_item_record",
                        side_effect=RuntimeError("oi idx fail"),
                    ):
                        with patch(
                            "lab_manager.services.search.index_product_record",
                            side_effect=RuntimeError("prod idx fail"),
                        ):
                            doc_mod._index_approved_doc(doc.id)
    ...
    def test_index_inventory_failure(self, db_session, tmp_path):
        """Lines 278-282: index_inventory_record raises."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        doc = Document(
            file_path="/tmp/test_invidx.png",
            file_name="invidx.png",
            status=DocumentStatus.approved,
        )
        db_session.add(doc)
        db_session.commit()
        ...
        with patch(
            ...
            with patch("lab_manager.services.search.index_document_record"):
                with patch("lab_manager.services.search.index_order_record"):
                    with patch("lab_manager.services.search.index_order_item_record"):
                        with patch(
                            "lab_manager.services.search.index_inventory_record",
                            side_effect=RuntimeError("inv idx fail"),
                        ):
                            doc_mod._index_approved_doc(doc.id)
    ...
    def test_index_outer_exception(self, tmp_path):
        """Lines 285-286: outer exception handler."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
    ...
    def test_index_no_order(self, db_session, tmp_path):
        """Lines 244-245: document has no linked order, returns early."""
        os.environ["UPLOAD_DIR"] = str(tmp_path / "uploads")
        os.environ["AUTH_ENABLED"] = "false"
        from lab_manager.config import get_settings
        ...
        def scalars_side_effect(stmt):
            ...
            if call_count["n"] == 1:
                ...
                result.first.return_value = None  # no order
            return result
