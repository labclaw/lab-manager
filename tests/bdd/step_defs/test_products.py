...
def get_product_nonexistent(api, pid):
    return api.get(f"/api/products/{pid}")
...
def list_products_for_vendor(api, ctx, vendor_name):
    v = ctx["vendors"][vendor_name]
    r = api.get("/api/products/", params={"vendor_id": v["id"]})
    assert r.status_code == 200, r.text
    return r.json()
...
def search_products(api, query):
    r = api.get("/api/products/", params={"search": query})
    assert r.status_code == 200, r.text
    return r.json()
...
def update_product_name(api, product, name):
    r = api.patch(f"/api/products/{product['id']}", json={"name": name})
    assert r.status_code == 200, r.text
    return r.json()
...
def list_product_orders(api, product):
    r = api.get(f"/api/products/{product['id']}/orders")
    assert r.status_code == 200, r.text
    return r.json()
