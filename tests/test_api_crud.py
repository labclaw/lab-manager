...
def test_vendor_update_not_found(client):
    resp = client.patch("/api/v1/vendors/999", json={"name": "X"})
...
def test_vendor_delete_not_found(client):
    resp = client.delete("/api/v1/vendors/999")
    assert resp.status_code == 404
...
def test_vendor_relationship_not_found(client):
    resp = client.get("/api/v1/vendors/999/products")
    assert resp.status_code == 404
...
def test_product_orders_relationship(client):
    ...
    resp = client.get(f"/api/v1/products/{pid}/orders")
    assert resp.json()["total"] == 1
...
def test_order_items_crud(client):
    ...
    # List items
    resp = client.get(f"/api/v1/orders/{oid}/items")
    assert resp.json()["total"] == 1

    # Get item
    resp = client.get(f"/api/v1/orders/{oid}/items/{iid}")
    assert resp.status_code == 200
    ...
    # Delete item
    resp = client.delete(f"/api/v1/orders/{oid}/items/{iid}")
    assert resp.status_code == 204
    resp = client.get(f"/api/v1/orders/{oid}/items/{iid}")
    assert resp.status_code == 404
...
def test_order_items_filter(client):
    ...
    resp = client.get(f"/api/v1/orders/{oid}/items?catalog_number=AB")
    assert resp.json()["total"] == 1

    resp = client.get(f"/api/v1/orders/{oid}/items?lot_number=LOT-B")
    assert resp.json()["total"] == 1
...
def test_inventory_update(client):
    ...
    iid = resp.json()["id"]
    resp = client.patch(f"/api/v1/inventory/{iid}", json={"quantity_on_hand": 5})
    assert resp.status_code == 200
...
def test_inventory_soft_delete(client):
    ...
    iid = resp.json()["id"]
    resp = client.delete(f"/api/v1/inventory/{iid}")
    assert resp.status_code == 204
    resp = client.get(f"/api/v1/inventory/{iid}")
    assert resp.status_code == 200
    assert resp.json()["status"] == "deleted"
