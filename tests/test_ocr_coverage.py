...
class TestGetOcrModel:
    def test_ocr_model_set(self):
        ...
        settings = MagicMock(ocr_model="nvidia_nim/meta/llama", extraction_model="gemini-2.5-flash")
...
class TestOcrLocal:
    ...
    def test_provider_base_url_override(self):
        ...
        settings = MagicMock(ocr_local_model="deepseek_ocr", ocr_local_url="http://localhost:8000")
        with (
            patch("lab_manager.intake.ocr.get_settings", return_value=settings),
...
class TestOcrGemini:
    def test_success(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch("lab_manager.intake.ocr.get_settings", return_value=MagicMock(
                extraction_api_key="", extraction_model="gemini-2.5-flash"
            )),
        ):
            mock_genai.Client.return_value = mock_client
            result = _ocr_gemini(img, MagicMock(extraction_api_key="", extraction_model="gemini-2.5-flash"))
    ...
    def test_no_api_key_raises(self, monkeypatch, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="No Gemini OCR key"):
            _ocr_gemini(img, MagicMock(extraction_api_key="", extraction_model="gemini-2.5-flash"))
...
class TestOcrNvidia:
    ...
    def test_success(self, monkeypatch, tmp_path):
        ...
        mock_resp.json.return_value = {"choices": [{"message": {"content": "nvidia ocr result"}}]}
        mock_httpx = _make_mock_httpx(post_return=mock_resp)
        with (
            patch.dict("sys.modules", {"httpx": mock_httpx}),
            patch("lab_manager.intake.ocr.get_settings", return_value=MagicMock(nvidia_build_api_key="nv-key")),
        ):
            result = _ocr_nvidia(img, MagicMock(nvidia_build_api_key="nv-key"), "nvidia_nim/meta/llama")
    ...
    def test_no_api_key_raises(self, monkeypatch, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="No NVIDIA OCR key"):
            _ocr_nvidia(img, MagicMock(nvidia_build_api_key=""), "nvidia_nim/meta/llama")
    ...
    def test_rate_limit_retry(self, monkeypatch, tmp_path):
        ...
        resp_ok.json.return_value = {"choices": [{"message": {"content": "after retry"}}]}
    ...
    def test_empty_choices_raises(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            _ocr_nvidia(img, MagicMock(nvidia_build_api_key="nv-key"), "nvidia_nim/meta/llama")
...
class TestOcrApi:
    def test_nvidia_model_direct(self, tmp_path):
        ...
        img = tmp_path / "test.png"
        img.write_bytes(b"fake")
        settings = MagicMock(ocr_model="nvidia_nim/meta/llama", extraction_model="gemini-2.5-flash", nvidia_build_api_key="")
        with patch("lab_manager.intake.ocr._ocr_nvidia", return_value="nvidia result") as mock:
    ...
    def test_gemini_success(self, tmp_path):
        ...
        img = tmp_path / "test.png"
        img.write_bytes(b"fake")
        settings = MagicMock(ocr_model="", extraction_model="gemini-2.5-flash", nvidia_build_api_key="")
        with patch("lab_manager.intake.ocr._ocr_gemini", return_value="gemini result") as mock:
    ...
    def test_gemini_fails_nvidia_fallback(self, tmp_path, monkeypatch):
        ...
        img = tmp_path / "test.png"
        img.write_bytes(b"fake")
        settings = MagicMock(ocr_model="", extraction_model="gemini-2.5-flash", nvidia_build_api_key="nv-key")
        with (
            patch("lab_manager.intake.ocr._ocr_gemini", side_effect=RuntimeError("fail")),
            patch("lab_manager.intake.ocr._ocr_nvidia", return_value="fallback result") as mock_nvidia,
        ):
...
class TestExtractTextFromImage:
    ...
    def test_tier_auto_local_fails_falls_to_api(self, tmp_path):
        ...
        with (
            patch("lab_manager.intake.ocr.get_settings", return_value=settings),
            patch("lab_manager.intake.ocr._ocr_local", side_effect=RuntimeError("local fail")),
    ...
    def test_invalid_tier_defaults_to_auto(self, tmp_path):
        ...
        with (
            patch("lab_manager.intake.ocr.get_settings", return_value=settings),
            patch("lab_manager.intake.ocr._ocr_local", side_effect=RuntimeError("fail")),
            patch("lab_manager.intake.ocr._ocr_api", return_value="api result"),
        ):
            assert extract_text_from_image(img) == "api result"
    ...
    def test_file_not_found_returns_empty(self):
        from lab_manager.intake.ocr import extract_text_from_image
        with patch("lab_manager.intake.ocr.get_settings", return_value=MagicMock(ocr_tier="local")):
    ...
    def test_api_error_returns_empty(self, tmp_path):
        ...
        with (
            patch("lab_manager.intake.ocr.get_settings", return_value=settings),
            patch("lab_manager.intake.ocr._ocr_api", side_effect=genai_errors.APIError(500, {"msg": "err"}, None)),
        ):
    ...
    def test_generic_exception_returns_empty(self, tmp_path):
        ...
        with (
            patch("lab_manager.intake.ocr.get_settings", return_value=settings),
            patch("lab_manager.intake.ocr._ocr_api", side_effect=RuntimeError("unexpected")),
        ):
            assert extract_text_from_image(img) == ""
