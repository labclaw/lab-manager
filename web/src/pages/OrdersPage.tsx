...
export function OrdersPage({ onError }: OrdersPageProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      {/* Breadcrumbs & Header */}
      <div>
        <div className="flex items-center text-xs text-slate-400 font-medium uppercase tracking-wider mb-2">
        ...
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-4xl font-extrabold text-[#e2e2e9] tracking-tight">Orders</h2>
            <p className="text-slate-400 mt-2 text-sm">
              Tracking {total} active shipments and pending procurement requisitions.
            </p>
          </div>
          <button className="bg-gradient-to-br from-primary to-[#6d28d9] text-white px-6 py-3 rounded-xl font-bold flex items-center shadow-lg hover:scale-105 transition-transform duration-200">
      ...
      {/* Tabs */}
      <div className="flex items-center space-x-8 border-b border-[#2d2d44]">
        {TABS.map((tab) => (
          <button
            key={tab.value}
            ...
            className={`pb-4 border-b-2 text-sm tracking-wide transition-all ${
              activeTab === tab.value
                ...
                : 'text-slate-400 font-medium border-transparent hover:text-[#e2e2e9]'
            }`}
          >
            {tab.label}
          </button>
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-base font-semibold text-[#e2e2e9]">No orders found</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {featured && (
            <div className="lg:col-span-12 bg-[#16162a] border border-[#2d2d44] rounded-xl p-8 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
              <div className="absolute top-0 left-0 w-1.5 h-full bg-primary" />
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-lg font-bold text-[#e2e2e9]">
                      Order #{featured.po_number ?? featured.id}
                    </h3>
                    <span className="bg-[#4e3b7f] text-[#e9ddff] px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button className="bg-[#2a2a44] text-primary px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-[#33334d] transition-colors">
                    Track Package
                  </button>
                </div>
              </div>
              ...
              <div className="relative pt-4 pb-2 px-2">
                <div className="absolute top-8 left-0 w-full h-1 bg-[#24243e] rounded-full overflow-hidden">
                  <div
                      ...
                      <div key={step.key} className="flex flex-col items-center">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center z-10 shadow-md ${
                          active || current
                            ...
                            : 'bg-[#24243e] text-slate-500/40 border border-[#2d2d44]'
                        }`}>
                        ...
                        <span className={`text-[10px] font-bold mt-3 uppercase tracking-tighter ${
                          active || current ? 'text-[#e2e2e9]' : 'text-slate-400 opacity-50'
                        }`}>
                          {step.label}
                        </span>
                      </div>
          ...
          {secondary.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className={`lg:col-span-6 bg-[#16162a] border rounded-xl p-6 shadow-sm flex flex-col justify-between group hover:shadow-md transition-all ${
                  isPending ? 'border-dashed border-2 border-[#2d2d44]' : 'border-[#2d2d44]'
                }`}
              >
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="h-12 w-12 rounded-xl bg-[#24243e] flex items-center justify-center group-hover:scale-110 transition-transform">
                    <span className={`material-symbols-outlined ${isPending ? 'text-slate-400' : 'text-primary'}`}>
                ...
                <div className="space-y-4">
                  {isPending ? (
                    <div className="p-3 bg-[#1e1e36] rounded-lg text-[11px] text-slate-400 italic">
                      Awaiting sign-off from Lab Manager head (Principal Investigator)
                    </div>
                  ) : (
                    <div className="flex justify-between items-end">
                      <div className="w-2/3 h-1.5 bg-[#24243e] rounded-full overflow-hidden">
                        <div className="h-full bg-primary w-[25%]" />
                      </div>
                      <span className="text-[10px] font-bold text-slate-400">25% Progress</span>
                    </div>
                  )}
                  <div className="flex gap-4 pt-2 border-t border-[#2d2d44]">
                    <button className={`flex-1 py-2 rounded-lg font-bold text-xs ${
                      isPending
                        ? 'bg-primary text-white shadow-sm'
                        : 'bg-[#2a2a44] text-primary'
                    }`}>
                      {isPending ? 'View Details' : 'Track'}
                    </button>
                    <button className="flex-1 text-slate-400 py-2 font-bold text-xs hover:text-[#e2e2e9] transition-colors">
                      Invoice
                    </button>
                  </div>
                </div>
          ...
          <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
            ...
            <div className="bg-[#24243e] border border-[#2d2d44] p-6 rounded-xl flex flex-col justify-between">
              <div>
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Delivery Success</h4>
                <p className="text-2xl font-extrabold text-[#e2e2e9]">98.4%</p>
              </div>
              <div className="h-1 w-full bg-[#2d2d44] mt-4 rounded-full overflow-hidden">
                <div className="h-full bg-primary w-[98%]" />
              </div>
            </div>
            <div className="bg-[#2a2a44] border border-[#2d2d44] p-6 rounded-xl flex flex-col justify-between">
              <div>
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Items in Transit</h4>
                <p className="text-2xl font-extrabold text-[#e2e2e9]">
