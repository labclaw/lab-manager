...
The system employs a five-stage architecture: (1)~optical character
recognition, (2)~parallel extraction by three frontier vision-language
models (VLMs), (3)~field-level consensus voting with automatic conflict
resolution, (4)~cross-model review where each model validates the merged
extraction, and (5)~rule-based validation with human review for unresolved
conflicts. In a benchmark of 10 real documents from 10 vendors, individual
VLMs achieved 90\% accuracy on vendor names and purchase order numbers,
while the consensus mechanism resolved most inter-model disagreements
...
laboratory operations platform with 88 API endpoints covering inventory,
...
with 16 relational models, a FastAPI~\citep{fastapi2024} backend with 88
REST API endpoints across 14 route modules, and a React frontend. The application provides Meilisearch for full-text search,
role-based access control (RBAC) with 7 permission levels, a
natural-language query interface (NL$\to$SQL via LiteLLM), CSV export,
and a complete audit trail. Users deploy it as a single Docker Compose
stack.

\subsection{Multi-model consensus pipeline}
\label{sec:pipeline}

The document intake pipeline processes each uploaded image through five
...
Lab Manager comprises approximately 71,000 lines of code: 13,800 lines
of Python backend (78 source files), 14,700 lines of TypeScript/React
frontend (53 component files), and 42,500 lines of Python tests (132 test
files, 700 test functions). The test suite includes unit tests, BDD
...
The database schema comprises 16 SQLModel tables covering documents,
vendors, products, orders (with line items), inventory (with lifecycle
operations: consume, transfer, adjust, dispose, open), locations,
equipment, staff, audit logs, alerts, notifications, and telemetry. All
...
The 88 API endpoints span 14 route modules (Table~\ref{tab:endpoints}),
...
Products      & 7  & CRUD, linked inventory \& orders \\
Orders        & 11 & CRUD, line items, receive shipment \\
Inventory     & 13 & CRUD, consume, transfer, adjust, low-stock, expiring \\
Search        & 2  & Full-text query, autocomplete suggestions \\
NL Query      & 2  & Natural language $\to$ SQL translation \\
Analytics     & 10 & Dashboard, spending, inventory value, top products \\
Export        & 4  & CSV: inventory, orders, products, vendors \\
Alerts        & 5  & Low-stock/expiry alerts, acknowledge, resolve \\
Audit         & 2  & Activity log, record history \\
Auth/Setup    & 7  & Login, setup wizard, session management \\
Team          & 5  & Invite, assign roles, deactivate \\
...
\textbf{Limitations.} The current benchmark is limited to 10 documents
from 10 vendors; a larger evaluation across 100+ documents and 50+ vendors
is needed to establish statistical confidence in accuracy metrics. The
consensus pipeline requires 3 API calls per document, which increases cost
and latency compared to single-model extraction; the tiered OCR strategy
and local model options partially mitigate this. Handwritten text
(particularly receiving dates) remains the primary failure mode for OCR,
with even the best local model (Apple Vision OCR) achieving only 1/5
accuracy on handwritten dates. The system has been tested with a limited
set of document types common in biomedical research laboratories;
generalization to chemistry, physics, or engineering supply documents
requires validation.
...
\begin{figure}[ht]
  ...
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}
    ...
    Docker Compose deployment. 88 API endpoints across 14 modules.
  \vspace{2.5cm}}}
  \caption{\textbf{Lab Manager system architecture.}
  The platform comprises a React frontend, FastAPI backend with 88 REST
  API endpoints, PostgreSQL database with 16 relational models, and
