...
Research laboratories are document-intensive. Supply shipments arrive
with packing lists, invoices, certificates of analysis (COAs), or shipping
labels that encode vendor information, purchase order numbers, catalog
numbers, lot numbers, quantities, and pricing. Researchers need this
information for inventory management, regulatory compliance, grant
accounting, and
...
\textbf{Related work in laboratory automation.} Beyond LIMS and ELN
systems, several recent efforts have applied AI to laboratory operations.
Boiko et al.~\citep{boiko2023autonomous} demonstrated autonomous chemical
research with GPT-4, and M.~Bran et al.~\citep{m2024augmenting} augmented
LLMs with chemistry tools. These systems focus on experiment planning and
execution rather than supply chain documentation. On the document
understanding side, DocParsing~\citep{boiko2023parsing} uses LLMs to parse
documents into structured formats, and ColPali~\citep{faysse2024colpali}
enables efficient document retrieval with VLMs. However, none of these
systems combine document understanding with a complete laboratory
management platform, and none employ multi-model consensus for extraction
reliability.
...
\textbf{Worked example.} Suppose three VLMs extract the vendor name
from a packing list: Claude extracts ``Thermo Fisher'', Gemini extracts
``Thermo Fisher Scientific'', and GPT-5.4 extracts ``Fisher Scientific''.
After normalization (lowercase, strip whitespace), Claude and Gemini agree
(``thermo fisher''), forming a majority group of size 2. The consensus
value is ``Thermo Fisher'' with agreement type ``majority'', and GPT-5.4's
value is recorded in the audit trail. The field is auto-accepted
...
No formal statistical hypothesis testing was performed. The 95\%
Clopper--Pearson exact binomial confidence intervals reported in
Table~\ref{tab:benchmark} quantify the uncertainty due to small sample
size ($n = 10$ documents). Pairwise model comparisons are reported as
non-overlapping confidence intervals rather than $p$-values, as the
sample size is insufficient for reliable significance testing. The
primary purpose of the benchmark is to characterize per-model accuracy
and identify the most error-prone extraction fields, not to establish
statistically significant performance differences between models. A
larger-scale evaluation ($n > 50$) with formal hypothesis testing is
...
The VLM benchmark data (10 documents, 4 models, per-field accuracy,
per-field ground truth annotations) is available in the repository at
...
Raw document images are not redistributed due to institutional data
policies, but any laboratory can reproduce the benchmark using their own
scanned documents with the provided evaluation framework.

% ============================================================
\section{Code Availability}
The complete source code is available at
\url{https://github.com/labclaw/lab-manager} under the Apache-2.0
license. No API keys are required for core functionality; the document
intake pipeline accepts configurable model providers, and the
evaluation framework operates on pre-extracted model outputs. The
repository includes the backend, Docker deployment configuration, test
suite, benchmark framework, and API documentation.

% ============================================================
\section{Acknowledgments}
We thank the members of Shen Lab at Massachusetts General Hospital
for testing the system with real laboratory documents and providing
feedback on extraction accuracy. We acknowledge Google DeepMind
(Gemini), Anthropic (Claude), OpenAI (GPT), Meta (Llama), NVIDIA
(NIM platform), and Mistral AI for providing API access to their
foundation models.
...
\begin{figure}[ht]
  ...
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{1.5cm}
    \textbf{[Figure 1: System architecture --- to be produced]}
  \vspace{1.5cm}}}
...
\begin{figure}[ht]
  \centering
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{1.5cm}
    \textbf{[Figure 2: Multi-model consensus pipeline --- to be produced]}
  \vspace{1.5cm}}}
...
\begin{figure}[ht]
  \centering
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{1.5cm}
    \textbf{[Figure 3: VLM benchmark results --- to be produced]}
  \vspace{1.5cm}}}
...
\begin{figure}[ht]
  \centering
  \fbox{\parbox{0.9\textwidth}{\centering\vspace{1.5cm}
    \textbf{[Figure 4: Application screenshots --- to be produced]}
  \vspace{1.5cm}}}
