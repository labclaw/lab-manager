...
def _validate_sql(sql: str) -> str:
    ...
    # Enforce a LIMIT clause on the *outer* query to prevent full-table scans.
    # Anchor to end-of-string so a LIMIT inside a subquery doesn't count.
    if not re.search(r"\bLIMIT\s+(\d+|ALL)\s*$", sql, re.IGNORECASE):
