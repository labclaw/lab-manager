...
def dashboard_summary(db: Session) -> dict:
    ...
    orders_by_status = dict(
        db.exec(select(Order.status, func.count(Order.id)).group_by(Order.status)).all()
    )
...
def inventory_value(db: Session) -> dict:
    ...
    item_count = db.exec(select(func.count(InventoryItem.id))).scalar() or 0
    return {
...
def order_history(
    ...
    return [
        ...
        for order, vendor_name, item_count, total_value in db.exec(q).all()
    ]
...
def inventory_report(db: Session, location_id: Optional[int] = None) -> list[dict]:
    ...
    return [
        ...
        for item, product_name, catalog_number, vendor_name, location_name in db.exec(
            q
        ).all()
    ]
...
def document_processing_stats(db: Session) -> dict:
    total = db.exec(select(func.count(Document.id))).scalar() or 0
