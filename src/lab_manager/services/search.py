...
def sync_products(db: Session) -> int:
    ...
    count = 0
    batch: list[dict] = []
    for product in db.exec(select(Product)).yield_per(_BATCH_SIZE):
...
def sync_vendors(db: Session) -> int:
    ...
    count = 0
    batch: list[dict] = []
    for v in db.exec(select(Vendor)).yield_per(_BATCH_SIZE):
        d: dict = {"id": v.id}
        if v.name:
            d["name"] = v.name
...
def sync_orders(db: Session) -> int:
    ...
    count = 0
    batch: list[dict] = []
    for order in db.exec(select(Order)).yield_per(_BATCH_SIZE):
...
def sync_order_items(db: Session) -> int:
    ...
    count = 0
    batch: list[dict] = []
    for item in db.exec(select(OrderItem)).yield_per(_BATCH_SIZE):
...
def sync_documents(db: Session) -> int:
    ...
    count = 0
    batch: list[dict] = []
    for doc in db.exec(select(Document)).yield_per(_BATCH_SIZE):
        d: dict = {"id": doc.id}
        if doc.file_name:
...
def sync_inventory(db: Session) -> int:
    ...
    count = 0
    batch: list[dict] = []
    for item in db.exec(select(InventoryItem)).yield_per(_BATCH_SIZE):
        d: dict = {"id": item.id}
        if item.lot_number:
